...
class FeedForward(LatentMetaDynamicsModel):
    def __init__(self, args):
        ...
        self.domain_seq = RnnEncoder(args, 
                                     ...
                                     n_layer=1,
                                     bd=False)
        self.domain = Aggregator(args.latent_dim, args.latent_dim, args.window, stochastic=False)
    ...
    def forward(self, x, xD, y, yD, name):
        ...
        # Domain encoding
        self.embeddings, self.mean_c, self.logvar_c = self.latent_domain(xD, yD, self.args.domain_size, name)
        ...
        _, self.mean_t, self.logvar_t = self.latent_domain(D_x_cat, D_y_cat, self.args.domain_size, name)
        return x_, z
